...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) BasicKit_M->Timing.taskTime0;
  BasicKit_step();
...
int main(int argc, char **argv)
{
  ...
  MX_ADC1_Init();
  rtmSetErrorStatus(BasicKit_M, 0);
  ...
  errorCode = extmodeInit(BasicKit_M->extModeInfo, &rtmGetTFinal(BasicKit_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(BasicKit_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(BasicKit_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
